...
<div class="row new-course-section">
    <div class="col-md-12">
        ...
        <div class="row tabs-live-session">
            <a href="#" class="col-md-6 new-live-session tab active"> New Live session </a>
            <a href="<?= site_url() ?>/calendar" class="col-md-6 all-live-sessions tab"> All Live Sessions </a>
        </div>

        <div class="courses-bottom mid-gray d-flex flex-column">


            <h2 class="text-center mt-3 mb-3 heading-font"> create live session </h2>
            <p class="text-center mt-5 mb-3"> Select a course and date from the below calendar </p>


            <form action="" method="post" class="new-meeting-form d-flex flex-column" id="new-meeting-form">

                <div class="d-flex justify-content-between align-content-center">
                    <select class="course-select mb-3 p-3" name="course-select">
                        <option value="" selected disabled> Select Course </option>
                        <?php if( !empty($instructor_courses['posts']) ): ?>
                            <?php foreach ( $instructor_courses['posts'] as $course ): ?>
...
<!--                            </div>-->
                ...
                <input type="hidden" id="meeting_date_utc" name="meeting_date_utc" value="">

                <div class="row mt-3 mb-3">
                    <div class="col-md-6">
                        <select name="timezone" id="timezone" class="mb-3">
                    ...
                    <div class="col-md-6">
                        <input type="number" min="1" max="999" name="meeting_duration" placeholder="duration in mins" class="meeting_duration">
                    </div>
                </div>


                <input type="text" class="meeting-title mb-3 p-3" name="meeting-title" placeholder="Live Session Title">

                <textarea class="mb-3 meeting-description" name="meeting-description" rows="5" placeholder="Live Session Description"></textarea>



                <input type="hidden" name="add_meeting_nonce" value="<?= wp_create_nonce( 'add_meeting_nonce' ) ?>" id="add_meeting_nonce">
                <a href="#" id="book-new-meeting" class="submit-btn p-4 d-flex justify-content-center align-content-center"> BOOK LIVE SESSION <i class="fa-solid fa-arrow-right"></i> </a>
...
<style>
    ...
    .new-meeting-form span.select2-selection.select2-selection--single {
        border-radius: 150px;
    }

    .new-meeting-form span.select2.select2-container.select2-container--default {
        width: 100% !important;
    }
...
<script src="<?= get_stylesheet_directory_uri() ?>/assets/js/angular.min.js"></script>
...
<script>
    ...
    // send date to backend to create meeting
    jQuery(document).on("click","#book-new-meeting", function(e) {
        e.preventDefault();
        $('.ajax-loader').fadeIn();
        var formData = new FormData(document.getElementById('new-meeting-form'));
        let add_meeting_nonce = $('#add_meeting_nonce').val();

        formData.append("action", "add_new_meeting");
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                if( response.message == 'success' ){
                    showSuccess('Live Session has been Added Successfully');
                    location.reload();
                } else {
    ...
    app.controller('dateTimeCtrl', function ($scope) {
        ...
        ctrl.updateDate = function (newdate) {
            ...
            jQuery('#meeting_date_utc').val( dateObj.toISOString() );
